...
/** Graphique croissance + badge FCFA — couleurs CapBudget (indigo / emerald) */
...
export default function CapBudgetLogo({ size = 'md', showText = false, className = '' }) {
    ...
    <div className={`flex items-center gap-2.5 ${className}`}>
      ...
      {showText && (
        <span className={`font-bold text-slate-800 tracking-tight ${s.word}`}>
          Cap<span className="text-indigo-600">Budget</span>
        </span>
      )}
    </div>
  );
...
export function FormBrandHeader({ title, subtitle }) {
  return (
    <div className="flex flex-col items-center gap-4 mb-6">
      <CapBudgetLogo size="lg" showText />
      <div className="text-center">
        <h2 className="text-slate-900 text-2xl font-bold tracking-tight">{title}</h2>
        {subtitle && (
          <p className="text-slate-500 text-sm mt-1.5 leading-relaxed max-w-xs">{subtitle}</p>
        )}
      </div>
    </div>
  );
