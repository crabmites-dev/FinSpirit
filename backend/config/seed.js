...
export const seedData = async (userId, db = pool) => {
    ...
    if (budgetCheck.rows.length === 0) {
        await db.query(`
            INSERT INTO budgets (user_id, name, category, budget_limit, spent, is_sample) VALUES
            ($1, 'Courses mensuelles', 'Courses',   50000, 35000, true),
            ($1, 'Transport',          'Transport', 20000,  8500,  true),
            ($1, 'Loisirs',            'Loisirs',   30000, 25000, true)
        `, [userId])
    }

    const objCheck = await db.query(
        'SELECT id FROM objectifs WHERE user_id = $1 AND is_sample = true LIMIT 1',
        [userId]
    )
    if (objCheck.rows.length === 0) {
        await db.query(`
            INSERT INTO objectifs (user_id, name, category, description, target, saved, deadline, is_sample) VALUES
            ($1, 'Voyage à Paris',  'Voyage',  'Vacances été 2027', 500000, 120000, '2027-06-01', true),
            ($1, 'Fonds d''urgence', 'Urgence', '3 mois de charges', 300000, 180000, null, true)
        `, [userId])
    }

    const echCheck = await db.query(
...
export const deleteSampleData = async (userId) => {
    ...
    await pool.query('DELETE FROM budgets WHERE user_id = $1 AND is_sample = true', [userId])
    await pool.query('DELETE FROM objectifs WHERE user_id = $1 AND is_sample = true', [userId])
...
export const hasSampleData = async (userId) => {
    const result = await pool.query(`
        SELECT (
            EXISTS (SELECT 1 FROM transactions WHERE user_id = $1 AND is_sample = true)
            OR EXISTS (SELECT 1 FROM budgets WHERE user_id = $1 AND is_sample = true)
            OR EXISTS (SELECT 1 FROM objectifs WHERE user_id = $1 AND is_sample = true)
            OR EXISTS (SELECT 1 FROM echeances WHERE user_id = $1 AND is_sample = true)
        ) AS has_sample
    `, [userId])
